...
public static class MeteoEndpoints
{
    public static IEndpointRouteBuilder MapMeteo(this IEndpointRouteBuilder app)
    {
        ...
        app.MapGet("/api/meteo", (HouseOsDbContext db) => LireAsync(db, DateTime.Now));

        return app;
    }

    /// <summary>
    /// Le document météo tel que l'UI le lit — partagé avec la vue e-ink
    /// (Features/Affichage). Une seule lecture d'horloge, fournie par l'appelant.
    /// </summary>
    public static async Task<MeteoDto> LireAsync(HouseOsDbContext db, DateTime instant)
    {
